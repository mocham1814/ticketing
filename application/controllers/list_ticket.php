...
function __construct(){
 ...
 function assignment()
 {
    ...
    if ($this->db->trans_status() === FALSE)
            ...
            } else 
            {
                ...
                $cek_dept = "SELECT reported FROM ticket WHERE id_ticket = '$id_ticket'";

                $reported = $this->db->query($cek_dept)->row()->reported;

                $cek = "SELECT email FROM karyawan WHERE nik = '$reported' LIMIT 1";
                $cek2 = "SELECT b.email, b.nama FROM teknisi a
                          LEFT JOIN karyawan b ON b.nik = a.nik
                          WHERE a.id_teknisi = '$id_teknisi' LIMIT 1";

                $toEmail = $this->db->query($cek)->row();
                $toEmail2 = $this->db->query($cek2)->row();

                $this->send_email($toEmail->email, $toEmail2->nama, $id_ticket);
                $this->send_email2($toEmail2->email, $id_ticket);
 ...
 public function send_email($toEmail, $nama, $ticket)
    {
      ...
      // Konfigurasi email
        ...
        // Subject email
        $this->email->subject('Ticket '.$ticket.' menunggu Approval teknisi');

        // Isi email
        $this->email->message("Ticket menunggu Approval teknisi ".$nama." , Terima kasih");
    ...
    public function send_email2($toEmail, $ticket)
    {
      ...
      // Konfigurasi email
        ...
        // Subject email
        $this->email->subject('Ticket '.$ticket.' ditugaskan kepada Anda');

        // Isi email
        $this->email->message("Ticket ".$ticket." ditugaskan kepada Anda, silahkan diproses, Terima kasih");
 ...
 public function pdflistticket()
    ...
    ob_start();
    $this->load->view('body/pdflistticket',$data);
    ...
    $pdf->WriteHTML($html);
    $pdf->Output('Report_Ticket.pdf', 'D');
