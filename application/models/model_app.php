...
class Model_app extends CI_Model{
    ...
    {
    $query = $this->db->query('SELECT A.nama, A.email, A.nik, A.alamat, A.jk, C.nama_dept, B.nama_jabatan
    ...
    public function datalist_ticket()
    {
        ...
        $query = $this->db->query("SELECT D.nama, E.nama_dept, A.status, A.id_ticket, A.tanggal, B.nama_sub_kategori, C.nama_kategori
                                   FROM ticket A 
    ...
    $query = $this->db->query("SELECT A.status, D.nama ,A.status, A.id_ticket, A.tanggal, B.nama_sub_kategori, C.nama_kategori 
        ...
        LEFT JOIN kategori C ON C.id_kategori = B.id_kategori
        LEFT JOIN karyawan D ON D.nik = A.reported 
        LEFT JOIN departemen E ON E.id_dept = D.id_dept WHERE E.id_dept = $id AND A.status = 1 OR  A.status= 0");
    return $query->result();
    }
    ...
    {
    $query = $this->db->query('SELECT * FROM departemen A LEFT JOIN departemen B ON B.id_dept = A.id_dept ');
    return $query->result();
    }
    ...
    {
     $query = $this->db->query("SELECT A.progress, A.tanggal_proses, A.status, A.problem_summary
        , B.nama, D.nama_kategori, F.nama_dept  FROM ticket A 
                                ...
                                LEFT JOIN departemen F ON F.id_dept = B.id_dept
                                WHERE id_teknisi = '$id'"                        
                                 );
    return $query->result();
    }
    ...
    public function dropdown_karyawan_it()
    {
        $sql = "SELECT A.nama, A.nik, B.nik as n FROM karyawan A 
                LEFT JOIN teknisi B on b.nik = a.nik
                LEFT JOIN departemen C ON C.id_dept = a.id_dept 
                WHERE c.nama_dept ='IT' AND b.nik IS NULL ORDER BY nama";
        $query = $this->db->query($sql);
